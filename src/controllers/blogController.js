...
const getBlog = async function (req, res) {
    try {
        ...
        res.status(200).send({ status: true, data: blog1, count: blogs });
    }
    catch (error) {
...
let deletedByQueryParams = async function (req, res) {
    try {
        ...
        let data = req.query
        if (Object.keys(data) != 0) {
            if (data.ispublished == true) { return res.status(400).send({ status: false, msg: "isPublished must be false. That is the requirement " }) }
